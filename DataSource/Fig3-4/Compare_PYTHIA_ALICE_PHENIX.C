void Compare_PYTHIA_ALICE_PHENIX()
{
	...
	LHCbJpsiPrompt->SetMarkerStyle(20);
	LHCbJpsiPrompt->SetMarkerSize(1);
	LHCbJpsiPrompt->SetMarkerColor(kOrange);
	LHCbJpsiPrompt->SetLineColor(kOrange);

	TGraphErrors * LHCbJpsiPromptSyst = new TGraphErrors(NLHCb,XValue,YValue,XValueErr,YValueSyst);
	...
	LHCbJpsiNP->SetMarkerStyle(20);
	LHCbJpsiNP->SetMarkerSize(1);
	LHCbJpsiNP->SetMarkerColor(kBlack);
	LHCbJpsiNP->SetLineColor(kBlack);


	TGraphErrors * LHCbJpsiNPSyst = new TGraphErrors(NLHCb,XValueNP,YValueNP,XValueErrNP,YValueSystNP);
...
*/
		...
		//	g1d_fvtxs_scaled[1]->SetFillColorAlpha(kGreen + 2, 0.3);
			...
			// oppo
			TLegend *leg1 = new TLegend(0.08, 0.95-0.06*4, 0.5, 0.95);
			leg1->SetFillStyle(0);
			leg1->SetTextSize(0.05);
			leg1->SetBorderSize(0);
